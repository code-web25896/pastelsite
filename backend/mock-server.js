...
const productInput = z.object({
  ...
  promoPrice: z.preprocess((v) => (v === '' || v === undefined ? null : v), z.number().nonnegative().max(9999999).nullable().optional()),
  promoCode: z.preprocess((v) => (v === '' || v === undefined ? null : typeof v === 'string' && !v.trim() ? null : v), z.string().trim().max(80).nullable().optional().transform((v) => (!v ? null : v.toUpperCase()))),
  promoDiscountPercent: z.preprocess((v) => (v === '' || v === undefined ? null : v == null ? null : Number(v)), z.number().min(0).max(100).nullable().optional()),
...
app.patch('/api/admin/products/:id', auth, admin, route(async (req, res) => {
  ...
  if (!product) return res.status(404).json({ error: 'Produit introuvable.' });
  if (updates.images && updates.images.some((img) => typeof img === 'string' && img.startsWith('data:image/'))) {
    cleanProductDiskImages(req.params.id);
  }
