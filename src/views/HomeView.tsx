...
export const HomeView: React.FC = () => {
  ...
  return (
    <div className="space-y-16 sm:space-y-24 pb-24 overflow-hidden">
      ...
      {/* 4. NOUVELLE SECTION PRO : UNIVERS & COLLECTIONS POPULAIRES */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-10 space-y-2">
          ...
          <h2 className="font-sans font-black text-2xl sm:text-4xl text-[#0B1833] tracking-tight">
            COLLECTIONS PHARES & TENDANCES
          </h2>
          <p className="text-sm text-gray-600">
            Trouvez en un clin d'œil les indispensables adaptés à vos besoins scolaires, artistiques et professionnels.
          </p>
        </div>
          ...
          {featuredCategories.map((cat, idx) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -6 }}
                onClick={cat.link}
                className="group relative bg-white rounded-3xl overflow-hidden border border-gray-200/80 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col justify-between"
              >
                {/* Top Image Preview */}
                <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0B1833]/70 via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity" />
                  
                  {/* Badge */}
                  <div className="absolute top-3 left-3">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm ${cat.badgeColor}`}>
                      {cat.badge}
                    </span>
                  </div>

                  {/* Floating Category Icon */}
                  <div className="absolute bottom-3 right-3 w-9 h-9 rounded-xl bg-white/90 backdrop-blur-md flex items-center justify-center text-[#0B1833] shadow-md group-hover:bg-[#8FD8C3] transition-colors">
                ...
                <div className="p-5 space-y-2 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-sans font-extrabold text-base text-[#0B1833] group-hover:text-[#B58BC5] transition-colors">
                      {cat.title}
                    </h3>
                  ...
                  <div className="pt-3 border-t border-gray-100 flex items-center justify-between text-xs font-bold text-[#0B1833]">
                    <span>Découvrir l'univers</span>
                    <ArrowRight className="w-4 h-4 text-[#8FD8C3] group-hover:translate-x-1.5 transition-transform" />
                  </div>
                </div>
              </motion.div>
            );
