...
export const AccountView: React.FC<AccountViewProps> = ({ initialTab = 'orders' }) => {
  ...
  const [address, setAddress] = useState(currentUser?.address || '23 Rue de la LibertÃ©, Menzah 5');
  ...
  const getStatusBadge = (status: Order['status']) => {
    switch (status) {
      ...
      case 'processing':
      case 'preparing':
        return <span className="bg-blue-100 text-blue-800 text-[11px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1"><Package className="w-3 h-3" /> En prÃ©paration</span>;
      case 'shipped':
      ...
      case 'delivered':
        return <span className="bg-emerald-100 text-emerald-800 text-[11px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> LivrÃ©e</span>;
      case 'cancelled':
        return <span className="bg-red-100 text-red-800 text-[11px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1"><XCircle className="w-3 h-3" /> AnnulÃ©e</span>;
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#0B1833] text-white flex items-center justify-center font-['Outfit'] font-black text-2xl shadow-md">
            {currentUser ? currentUser.firstName.charAt(0) : 'A'}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-['Outfit'] font-black text-xl sm:text-2xl text-[#0B1833]">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {currentUser ? currentUser.email : 'amira.b@outlook.com'} â€¢ Membre Espace Pastel
            </p>
          </div>
        </div>
          ...
          {activeTab === 'orders' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                <h2 className="font-['Outfit'] font-black text-xl text-[#0B1833]">
              ...
              {userOrders.length === 0 ? (
                <div className="py-12 text-center space-y-3">
                  ...
                  <p className="text-xs text-gray-500">Vous n'avez pas encore passÃ© de commande.</p>
                  <button
                  ...
                  {userOrders.map(order => (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-gray-200/60">
                        <div>
                          <span className="font-['Outfit'] font-black text-sm text-[#0B1833] block">
                            Commande #{order.orderNumber}
                          </span>
                          <span className="text-[11px] text-gray-500">
                            PassÃ©e le {new Date(order.createdAt).toLocaleDateString('fr-TN')}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          {getStatusBadge(order.status)}
                          <span className="font-['Outfit'] font-black text-sm text-[#0B1833]">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto py-1">
                          {order.items.map((item, idx) => (
                            <img
                              ...
                              title={`${item.productName} (Ã—${item.quantity})`}
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-6 max-w-xl">
              <div className="pb-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500">
                  GÃ©rez vos adresses de livraison par dÃ©faut en Tunisie.
                </p>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#0B1833] mb-1">TÃ©lÃ©phone de contact</label>
                <input
                  type="tel"
                  value={phone}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#0B1833] mb-1">Ville / RÃ©gion</label>
                  <input
                    type="text"
                    value={city}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-[#0B1833]/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 last:border-none">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-gray-500">{formatPrice(item.price)} Ã— {item.quantity}</div>
                    </div>
                  </div>
            ...
            {/* Total */}
            <div className="pt-3 border-t border-gray-100 flex justify-between font-['Outfit'] font-black text-base text-[#0B1833]">
